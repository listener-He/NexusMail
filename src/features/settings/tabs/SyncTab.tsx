    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h3 className="text-lg font-medium text-white">{t('settings.sync.title', 'Sync Status')}</h3>
            ...
            <div className="p-6 rounded-xl bg-white/5 border border-white/10 space-y-4">
                <div className="flex justify-between text-sm text-gray-400">
                    <span>{t('settings.sync.syncing', 'Syncing...')}</span>
                    <span>45%</span>
                </div>
                <div className="w-full h-2 bg-white/10 rounded-full overflow-hidden">
                    <div className="h-full bg-blue-500 w-[45%]"></div>
                </div>
                <p className="text-xs text-gray-500">{t('settings.sync.last_synced', 'Last synced: Just now')}</p>
            </div>

            <div className="space-y-2">
                <h3 className="text-lg font-medium text-white">{t('settings.sync.channels', 'Channels')}</h3>
                    ...
                    {['S3 Backup', 'WebDAV'].map(channel => (
                        <div key={channel} className="p-4 rounded-xl bg-white/5 border border-white/10 flex justify-between items-center">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm text-gray-400">{t('settings.sync.active', 'Active')}</span>
                            </div>
                        </div>
                    ))}
                </div>
